...
?>
<table cellspacing="0" cellpadding="6" style="width: 100%; border: 1px solid #eee;border-collapse: collapse; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif; background: #fafafa;">
	<thead>
	<tr class="order_item">
	...
	if ( ! empty( $items ) ) :
			...
			<tr class="order_item">
				...
				if ( get_option( 'ywraq_show_preview' ) == 'yes' ) :
					...
					<td scope="col" class="td" style="text-align:center;border: 1px solid #eee;">
						...
						?>
						<a href="<?php echo esc_url( $_product->get_permalink() ); ?>"><img src="<?php echo esc_url( $src ); ?>" height="<?php echo esc_attr( $height ); ?>" width="<?php echo esc_attr( $width ); ?>" style="margin-right: 0;" /></a>
					</td>
				<?php endif ?>
		...
		if(!$hide_price) :
			...
			foreach ( $order->get_order_item_totals() as $key => $total ) {
				?>
				<tr class="order_item tfoot">
					<th scope="col" colspan="<?php echo esc_attr( $colspan ); ?>" style="text-align:left;border: 1px solid #eee;"><?php echo wp_kses_post( $total['label'] ); ?></th>
